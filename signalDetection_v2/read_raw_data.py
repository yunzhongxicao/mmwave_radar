...
if __name__ == "__main__":
    ...
    para_path = 'E:\\基于毫米波的运动监测\\device_data\\2021_1_30_11_33_58_data.txt'
    data_path = 'E:\\基于毫米波的运动监测\\device_data\\2021_1_30_11_33_58_data.npy'
    ...
    # 作出对应的频谱图
    plt.plot(f_doppler[2:], radar_data_1_doppler_fft_amp_body[2:])
    plt.xlim(0, 3)
    ...
    plt.show()
    # 求出对应范围内的频率
    heartbeat_frequency_1 = 0
    respire_frequency_1 = 0
    heartbeat_amp = 0
    respire_amp = 0
    for i in range(len(f_doppler)):
        if 0.2 <= f_doppler[i] <= 0.9:
            if radar_data_1_doppler_fft_amp_body[i] > respire_amp \
                    and radar_data_1_doppler_fft_amp_body[i] > radar_data_1_doppler_fft_amp_body[i - 1] \
                    and radar_data_1_doppler_fft_amp_body[i] > radar_data_1_doppler_fft_amp_body[i + 1]:
                respire_amp = radar_data_1_doppler_fft_amp_body[i]
                respire_frequency_1 = f_doppler[i]

        elif 1 <= f_doppler[i] <= 1.6:
            if radar_data_1_doppler_fft_amp_body[i] > heartbeat_amp \
                    and radar_data_1_doppler_fft_amp_body[i] > radar_data_1_doppler_fft_amp_body[i - 1] \
                    and radar_data_1_doppler_fft_amp_body[i] > radar_data_1_doppler_fft_amp_body[i + 1]:
                heartbeat_amp = radar_data_1_doppler_fft_amp_body[i]
                heartbeat_frequency_1 = f_doppler[i]

    print("呼吸频率：", round(respire_frequency_1, 2), "---", round(respire_frequency_1 * 60, 2))
    print("心跳频率：", round(heartbeat_frequency_1, 2), "---", round(heartbeat_frequency_1 * 60, 2))
    ...
    plt.show()
    # 求出频率
    heartbeat_frequency_2 = 0
    respire_frequency_2 = 0
    heartbeat_amp = 0
    respire_amp = 0
    for i in range(len(f_doppler)):
        if 0.2 <= f_doppler[i] <= 0.9:
            if radar_data_1_doppler_fft_amp_body[i] > respire_amp \
                    ...
                    and radar_data_1_doppler_fft_amp_body[i] > radar_data_1_doppler_fft_amp_body[i + 1]:
                respire_amp = radar_data_1_doppler_fft_amp_body[i]
                respire_frequency_2 = f_doppler[i]
            ...
            if radar_data_1_doppler_fft_amp_body[i] > heartbeat_amp \
                    ...
                    and radar_data_1_doppler_fft_amp_body[i] > radar_data_1_doppler_fft_amp_body[i + 1]:
                heartbeat_amp = radar_data_1_doppler_fft_amp_body[i]
                heartbeat_frequency_2 = f_doppler[i]

    print("呼吸频率：", round(respire_frequency_2, 2), "---", round(respire_frequency_2 * 60, 2))
    print("心跳频率：", round(heartbeat_frequency_2, 2), "---", round(heartbeat_frequency_2 * 60, 2))
    ...
    # 画出第一次FFT后相位时间图
    plt.plot(radar_data_1_fft_phase_body)
    plt.show()
