...
if __name__ == "__main__":
	...
	for i in range(0, d_num):
		dem_i = dem[2, i * d_l:(i + 1) * d_l]
		print(dem_i.shape)

		# standard
		dem_i = signal.detrend(dem_i)
		m_i = dem_i.mean()
		dem_i = dem_i - m_i

		# fft
		sp = np.fft.fft(dem_i)
		f = np.arange(int(d_l / 2)) * Fs / d_l

		# 滤波
		test_y = sp  # 滤波后频域信号
		for j in range(len(sp)):
			if j >= (len(sp) - 5) or j <= 5:
				test_y[j] = 0

		P2 = np.abs(test_y / d_l) * 2  # 滤波后频域信号幅值
		P1 = P2[range(int(d_l / 2))]
		test = np.fft.ifft(test_y)  # 滤波后反变换时域信号
		...
		# # 相位
		# phase_i = np.angle(test_y[max_index], deg=True)
		# phase_all = phase_all.append(phase_i)


	#
	# T_d_sample = devconf.chirp_to_chirp_time_100ps*(10**(-10))  # 距离d的采样周期
	# f_d_sample = 1/T_d_sample
	#
	# # 利用频谱测出的距离
	# plt.plot(T_d_sample*range(len(d_all)),d_all)
	# plt.show()
	#
	# # 相位测出距离
	# lamda = 2
	# d_phase = phase_all*lamda/(4*np.pi)
	# plt.plot(T_d_sample*range(len(d_all)),d_phase)
	# plt.show()
